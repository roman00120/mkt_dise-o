...
function handle_comment()
{
    ...
    if ($_SESSION['user_type'] !== 'admin' && $ticket['department'] !== ($_SESSION['department'] ?? null)) {
        http_response_code(403);
...
function handle_upload()
{
    ...
    $ticket = get_ticket_by_id($id);
    if (! $ticket || empty($_FILES['file'])) {
        http_response_code(400);
        echo json_encode(['error' => 'Ticket o archivo requerido']);
        exit;
    }
    if ($_FILES['file']['error'] !== UPLOAD_ERR_OK || $_FILES['file']['size'] > 100 * 1024 * 1024) {
        http_response_code(400);
        echo json_encode(['error' => 'Archivo inválido o mayor a 100 MB']);
        exit;
    }
    ...
    if (! in_array($mime, $mimeAllowed, true)) {
        http_response_code(400);
        echo json_encode(['error' => 'El contenido del archivo no coincide con un tipo permitido']);
        exit;
    }
    $dir = DATA_PATH.'/attachments/'.$id;
    if (! is_dir($dir)) {
        mkdir($dir, 0750, true);
    }
    ...
    foreach ($tickets as &$t) {
        if ($t['id'] == $id) {
            ...
            $t['attachments'][] = ['name' => basename($_FILES['file']['name']), 'file' => $name, 'size' => $_FILES['file']['size'], 'at' => date('Y-m-d H:i:s')];
...
function handle_download()
{
    ...
    $ticket = get_ticket_by_id($id);
    if (! $ticket || ! $file || ($_SESSION['user_type'] !== 'admin' && $ticket['department'] !== ($_SESSION['department'] ?? null))) {
        http_response_code(403);
        exit;
    }
    $path = DATA_PATH.'/attachments/'.$id.'/'.$file;
    if (! is_file($path)) {
        http_response_code(404);
        exit;
    }
