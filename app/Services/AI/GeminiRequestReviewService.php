...
class GeminiRequestReviewService
{
    public function review(CreativeRequest $request): array
    {
        ...
        if (filled(config('gemini.ca_bundle'))) {
            $http = $http->withOptions(['verify' => config('gemini.ca_bundle')]);
        }
        ...
        if (! is_string($text) || blank($text)) {
            throw new RuntimeException('Gemini no devolvió una revisión.');
        }
    ...
    {
        if (! is_array($result) || ! in_array($result['status'] ?? null, ['ready', 'warning', 'incomplete'], true)) {
            throw new RuntimeException('La respuesta de Gemini no cumple el formato esperado.');
        }
        foreach (['summary', 'corrected_text', 'spelling_corrections', 'missing_information', 'ambiguous_instructions', 'contradictions', 'recommendations'] as $key) {
            if (! array_key_exists($key, $result) || (! is_string($result[$key]) && ! is_array($result[$key]))) {
                throw new RuntimeException('La respuesta de Gemini está incompleta.');
            }
        }
    }
}
